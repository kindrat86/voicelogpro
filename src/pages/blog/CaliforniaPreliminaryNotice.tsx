...
import californiaPreliminaryNotice from "@/content/blog/california-preliminary-notice";

export default function CaliforniaPreliminaryNoticePage() {
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>{californiaPreliminaryNotice.title} | VoiceLogPro</title>
        <meta name="description" content={californiaPreliminaryNotice.metaDescription} />
        <meta name="keywords" content={californiaPreliminaryNotice.keywords.join(", ")} />
        <link rel="canonical" href={`https://voicelogpro.com/blog/${californiaPreliminaryNotice.slug}`} />
        <meta property="og:title" content={californiaPreliminaryNotice.title} />
        <meta property="og:description" content={californiaPreliminaryNotice.metaDescription} />
        <meta property="og:type" content="article" />
      </Helmet>
      
      <JsonLd schema={californiaPreliminaryNotice.faqSchema} />
      ...
      <article className="container max-w-4xl px-4 py-12">
        <header className="mb-12">
          <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-4">
            <span className="flex items-center gap-1">
              ...
              {californiaPreliminaryNotice.jurisdiction}
            </span>
            <span className="flex items-center gap-1">
              <Calendar className="h-4 w-4" />
              {new Date(californiaPreliminaryNotice.publishedAt).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
              })}
            ...
            <span className="flex items-center gap-1">
              ...
              {californiaPreliminaryNotice.targetAudience}
            </span>
          </div>
          
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-foreground mb-6">
            {californiaPreliminaryNotice.title}
          </h1>
          
          <p className="text-lg text-muted-foreground leading-relaxed">
            {californiaPreliminaryNotice.metaDescription}
          </p>
        </header>
        ...
        ">
          <MarkdownContent content={californiaPreliminaryNotice.content} />
        </div>

        <section className="mt-16 p-8 bg-muted/50 rounded-lg border border-border">
          <h2 className="text-2xl font-bold text-foreground mb-4">
            Never Miss a California Lien Deadline
          </h2>
        ...
        <footer className="mt-12 pt-8 border-t border-border">
          <p className="text-sm text-muted-foreground">
            ...
            California Civil Code Sections 8000-8200 are subject to change. Consult a licensed California attorney for specific legal guidance 
