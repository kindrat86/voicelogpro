...
import dailyLogBestPractices from "@/content/blog/construction-daily-log-best-practices";

export default function DailyLogBestPracticesPage() {
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>{dailyLogBestPractices.title} | VoiceLogPro</title>
        <meta name="description" content={dailyLogBestPractices.metaDescription} />
        <meta name="keywords" content={dailyLogBestPractices.keywords.join(", ")} />
        <link rel="canonical" href={`https://voicelogpro.com/blog/${dailyLogBestPractices.slug}`} />
        <meta property="og:title" content={dailyLogBestPractices.title} />
        <meta property="og:description" content={dailyLogBestPractices.metaDescription} />
        <meta property="og:type" content="article" />
      </Helmet>
      
      <JsonLd schema={dailyLogBestPractices.faqSchema} />
      ...
      <article className="container max-w-4xl px-4 py-12">
        <header className="mb-12">
          <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-4">
            <span className="flex items-center gap-1">
              ...
              {dailyLogBestPractices.jurisdiction}
            </span>
            <span className="flex items-center gap-1">
              <Calendar className="h-4 w-4" />
              {new Date(dailyLogBestPractices.publishedAt).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
              })}
        ...
        ">
          <MarkdownContent content={dailyLogBestPractices.content} />
        </div>

        <section className="mt-16 p-8 bg-muted/50 rounded-lg border border-border">
          <h2 className="text-2xl font-bold text-foreground mb-4">
            Start Creating Court-Ready Daily Logs
          </h2>
          <p className="text-muted-foreground mb-6">
            VoiceLogPro automates daily log creation with GPS-timestamped entries that are court-admissible as business records.
          </p>
        ...
        <footer className="mt-12 pt-8 border-t border-border">
          <p className="text-sm text-muted-foreground">
            ...
            Evidentiary rules vary by jurisdiction. Consult a licensed attorney for specific documentation requirements.
          </p>
        </footer>
      </article>
